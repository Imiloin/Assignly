...
#assignment(
  ...
  #multi-part(
    ...
    (
      (
        [Write the recurrence relation for this algorithm's time complexity],
        [T(n) = T(n-1) + T(n-2) + O(1)],
      ),
      (
        [Solve the recurrence to find the time complexity],
        [T(n) = O(φⁿ) where φ = (1+√5)/2 ≈ 1.618],
      ),
      (
        [Suggest an optimization and analyze its time complexity],
        [Use dynamic programming with memoization: O(n) time, O(n) space],
      ),
      (
